...
function insert(item, msecs, start = binding.getLibuvNow()) {
  ...
  if (list === undefined) {
    ...
    timerListMap.set(msecs, list = new TimersList(expiry, msecs));
...
function getTimerCallbacks(runNextTicks) {
  ...
  function listOnTimeout(list, now) {
      ...
      if (!timer._onTimeout) {
        if (!timer._destroyed) {
          ...
          if (timer[kHasPrimitive])
            knownTimersById.delete(asyncId);
    ...
    // before destroying.
    if (timerListMap.get(msecs) === list) {
